...
export async function GET() {
  const sprite = JSON.stringify(HB240_1 + HB240_2 + HB240_3 + HB240_4 + HB240_5)
...
.page-image{position:absolute;inset:0;overflow:hidden;background:#fff}
.page-image img{position:absolute;left:0;width:100%;height:700%;max-width:none;display:block;user-select:none;pointer-events:none}
...
function imageMarkup(i){if(i===null)return '<div class="blank">NGUYEN ARCHITECTURE<br>&amp; ENGINEERING</div>';return '<div class="page-image" data-page="'+i+'" role="img" aria-label="'+labels[i]+'"></div>'}
function hydrateImages(root){root.querySelectorAll('.page-image').forEach(box=>{if(box.querySelector('img'))return;const page=Number(box.getAttribute('data-page'));const img=document.createElement('img');img.alt='';img.draggable=false;img.src=sprite;img.style.top=String(page*-100)+'%';box.appendChild(img)})}
function pageMarkup(i,side){return '<div class="page '+side+'">'+imageMarkup(i)+'</div>'}
function render(){const pair=spreads[index];spread.innerHTML=pageMarkup(pair[0],'left')+pageMarkup(pair[1],'right');hydrateImages(spread);prevBtn.disabled=index===0;nextBtn.disabled=index===spreads.length-1}
function goNext(target=index+1){if(busy||index>=spreads.length-1)return;busy=true;nextFront.innerHTML=imageMarkup(spreads[index][1]);nextBack.innerHTML=imageMarkup(spreads[target][0]);hydrateImages(nextFront);hydrateImages(nextBack);turnNext.classList.remove('anim-next');void turnNext.offsetWidth;turnNext.classList.add('anim-next');setTimeout(()=>{index=target;render()},360);setTimeout(()=>{turnNext.classList.remove('anim-next');busy=false},740)}
function goPrev(target=index-1){if(busy||index<=0)return;busy=true;prevFront.innerHTML=imageMarkup(spreads[index][0]);prevBack.innerHTML=imageMarkup(spreads[target][1]);hydrateImages(prevFront);hydrateImages(prevBack);turnPrev.classList.remove('anim-prev');void turnPrev.offsetWidth;turnPrev.classList.add('anim-prev');setTimeout(()=>{index=target;render()},360);setTimeout(()=>{turnPrev.classList.remove('anim-prev');busy=false},740)}
