...
requireText(socalSource, 'const MOBILE_RENDER_FAILSAFE =', 'Concept 1 wrapper must keep the phone/tablet inline-opacity failsafe')
requireText(socalSource, '@media (max-width: 1024px)', 'wrapper failsafe must cover tablet-width viewports')
requireText(socalSource, '#main [style*="opacity:0"]', 'wrapper failsafe must target Framer wrappers stuck at opacity:0')
requireText(socalSource, '${MOBILE_RENDER_FAILSAFE}', 'wrapper failsafe must be injected into Concept 1 HTML')
