...
const offers: Offer[] = [
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1559028012-481c04fa702d?auto=format&fit=crop&w=1200&q=86',
    href: '/#portfolio',
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1557200134-90327ee9fafa?auto=format&fit=crop&w=1200&q=86',
    href: '/contact',
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=1200&q=86',
    href: '/contact',
    ...
    <section id="services" className="scroll-mt-20 overflow-hidden bg-[#f4f4f1] px-4 py-24 sm:px-6 lg:px-8">
      <style>{`
        ...
        @keyframes serviceGlow {
          0%, 100% { opacity: .3; transform: translate3d(-8%, 0, 0) scale(1); }
          50% { opacity: .58; transform: translate3d(8%, -4%, 0) scale(1.12); }
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          .service-card-rise,
          .service-card-glow { animation: none !important; }
        }
      `}</style>
      ...
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-12 grid gap-6 lg:grid-cols-[1fr_0.7fr] lg:items-end">
          ...
          <p className="max-w-xl text-base leading-relaxed text-black/58 lg:ml-auto">
        ...
        <div
          ref={trackRef}
          className="visual-service-track flex snap-x snap-mandatory gap-5 overflow-x-auto pb-4 lg:gap-6"
        >
          {offers.map((offer, index) => (
            <Link
              key={offer.title}
              href={offer.href}
              className="service-card-rise group relative min-h-[500px] min-w-[84vw] snap-start overflow-hidden rounded-[1.6rem] bg-neutral-950 text-white shadow-[0_20px_50px_rgba(0,0,0,.12)] transition duration-500 hover:-translate-y-2 hover:shadow-[0_32px_75px_rgba(0,0,0,.22)] sm:min-w-[390px] lg:min-w-[calc((100%-4.5rem)/4)]"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <img
                src={offer.image}
                alt={`${offer.title} service preview`}
              ...
              <div className="relative flex min-h-[500px] flex-col p-7 sm:p-8">
                <div>
                  ...
                  <p className="mt-3 max-w-sm text-sm leading-relaxed text-white/72">{offer.description}</p>
                </div>

                <div className="mt-auto">
                  <div className="mb-5 inline-flex rounded-full border border-white/25 bg-black/20 px-4 py-2 text-xs text-white/80 backdrop-blur-md">
                    {offer.detail}
                  </div>
        ...
        <div className="mt-7 flex items-center justify-between gap-5">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => move(-1)}
              className="flex h-12 w-12 items-center justify-center rounded-full border border-black/15 bg-white text-black transition hover:border-black hover:bg-black hover:text-white"
